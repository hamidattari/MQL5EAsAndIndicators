...
input color                  InpHighColor        = clrDodgerBlue; // Defined High color
input color                  InpLowColor         = clrTomato;     // Defined Low color
...
void UpdateDailyLevels()
  {
   ...
   if(InpMethod == METHOD_3)
   ...
   else
     {
      ...
      int hiShift = iHighest(_Symbol, _Period, MODE_HIGH, InpLookbackN, winStart);
      int loShift = iLowest(_Symbol, _Period, MODE_LOW,  InpLookbackN, winStart);
      if(hiShift < 0 || loShift < 0)
         return;

      double baseHigh = iHigh(_Symbol, _Period, hiShift);
      double baseLow  = iLow(_Symbol, _Period, loShift);
      newHigh = baseHigh;
      newLow  = baseLow;

      if(InpMethod == METHOD_2 && InpUpdateMode == UPDATE_MAIN_HL)
        {
         // Chronology: LARGER shift = EARLIER in time
         if(loShift > hiShift)
           {
            //--- Scenario A: Base Low occurred BEFORE Base High
            //    examine candles AFTER Base High up to end of window
            int cnt = hiShift - winStart;               // number of candles after base-high candle
            if(cnt > 0)
              {
               int bearish = 0;
               for(int s = winStart; s < hiShift; s++)
                  if(iClose(_Symbol, _Period, s) < iOpen(_Symbol, _Period, s))
                     bearish++;
               if(bearish >= InpUpdateX)
                 {
                  int updLoShift = iLowest(_Symbol, _Period, MODE_LOW, cnt, winStart);
                  if(updLoShift >= 0)
                     newLow = iLow(_Symbol, _Period, updLoShift);   // Updated Low
                 }
              }
           }
         else if(hiShift > loShift)
           {
            //--- Scenario B: Base High occurred BEFORE Base Low
            //    examine candles AFTER Base Low up to end of window
            int cnt = loShift - winStart;
            if(cnt > 0)
              {
               int bullish = 0;
               for(int s = winStart; s < loShift; s++)
                  if(iClose(_Symbol, _Period, s) > iOpen(_Symbol, _Period, s))
                     bullish++;
               if(bullish >= InpUpdateX)
                 {
                  int updHiShift = iHighest(_Symbol, _Period, MODE_HIGH, cnt, winStart);
                  if(updHiShift >= 0)
                     newHigh = iHigh(_Symbol, _Period, updHiShift); // Updated High
                 }
              }
           }
        }
   ...
   if(g_tradesToday == 1)
     {
      ...
      else if(lastReason == DEAL_REASON_TP)
        {
...
void DrawLevelLines(const datetime dayStart)
  {
   ...
   string highName = OBJ_PREFIX + "highgh_" + dayTag;
   string lowName = OBJ_PREFIX + "loww_"  + dayTag;
   datetime tEnd = dayStart + 86400;

   ObjectsDeleteAll(0, OBJ_PREFIX);   // keep only the current day's lines

   ObjectCreate(0, highName, OBJ_TREND, 0, g_lineStartTime, g_definedHigh, tEnd, g_definedHigh);
   ...
   ObjectCreate(0, lowName, OBJ_TREND, 0, g_lineStartTime, g_definedLow, tEnd, g_definedLow);
